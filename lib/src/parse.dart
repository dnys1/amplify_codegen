...
Map<String, Model> parseSchema(String schema) {
  ...
  for (var modelDefinition in _models.values) {
    ...
    var updatedModel = model.rebuild((m) {
    ...
    updatedModel = updatedModel.rebuild((m) {
      m.fields.map((f) => f.rebuild((field) => field.ignore =
          (field.ignore ?? false) || ignoreFields.contains(f.name)));
    });
