...
void main() {
  ...
  group('ReCase', () {
    ...
    group('Header-Case', () {
      ...
      test('from "$mockText", using String extension.', () {
        expect(
            mockText.headerCase, equals('This-Is-Some-Sample-Text-You-Dig?'));
      });
    });
    ...
    group('Sentence case', () {
      ...
      test('from "${rcInput.originalText}".', () {
        expect(
            rcInput.sentenceCase, equals('This is some sample text you dig?'));
      });
    ...
    group('CONSTANT_CASE', () {
      ...
      test('from "${rcInput.originalText}".', () {
        expect(
            rcInput.constantCase, equals('THIS_IS_SOME_SAMPLE_TEXT_YOU_DIG?'));
      });
    ...
    group('Preserve Acronyms', () {
      test('JSON', () {
        const modelName = 'MyJSONType';

        expect(modelName.camelCase, equals('myJsonType'));
        expect(modelName.pascalCase, equals('MyJsonType'));
        expect(modelName.constantCase, equals('MY_JSON_TYPE'));
        expect(modelName.snakeCase, equals('my_json_type'));
      });

      test('AWS', () {
        const modelName = 'AWSDateTime';

        expect(modelName.camelCase, equals('awsDateTime'));
        expect(modelName.pascalCase, equals('AwsDateTime'));
        expect(modelName.constantCase, equals('AWS_DATE_TIME'));
        expect(modelName.snakeCase, equals('aws_date_time'));
      });
    });
  });
}
